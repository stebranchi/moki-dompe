...
// And it is given to the function createV4SelectableForceDirectedGraph
	...
	function graphAll() {
        ...
        if (!error) {
        	...
        	$scope.createV4SelectableForceDirectedGraph(svg, graph);
        } else {
            console.error(error);
        }
    })
...
// And it creates the graph of all nodes that are in the json
	$scope.createV4SelectableForceDirectedGraph = function(svg, graph) {
	    ...
	    svg.selectAll('text')
	        ...
	        .attr('y', function(d,i) { return parentHeight - (1-i) * 18; })
